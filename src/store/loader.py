...
from src.store.schema import ET, TABLES, AS_OF_COL
...
def get_features(day,
    ...
    if pd.api.types.is_datetime64_any_dtype(df[as_of_col]):
        df = df[df[as_of_col] <= cutoff]
    else:
        df = df[df[as_of_col] <= cutoff.tz_convert(ET).date()]

    return df.sort_values(as_of_col).reset_index(drop=True)
